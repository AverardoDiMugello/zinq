...
fn main() {
    ...
    // Initialize
    vm.init_mem(0x80000000, &fs::read("sail_bins/bootloader.bin").unwrap());
    vm.init_mem(0x81000000, &fs::read("sail_bins/sail.dtb").unwrap());
    ...
    let emu = StepEmu::new(vm);
    let sys = emu.run(Some(24587251));
...
impl Tube {
    ...
    fn mmio_write(&mut self, _: usize, data: &[u8], _: &mut DevCtx) -> MemWriteResult {
        if data.len() > 1 {
            ...
            print!("{0}", *data.first().unwrap() as char);
            MemWriteResult(Some(()))
        }
    }
}
